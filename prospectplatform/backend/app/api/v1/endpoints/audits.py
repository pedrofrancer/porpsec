...
from app.schemas.audit import AuditRead, DiagnosisReport, DiagnosisItem, DiagnosisDigitalPresence, DiagnosisCommunication, DiagnosisRegional, DiagnosisRecommendation
...
def get_diagnosis(company_id: int, db: Session = Depends(get_db)):
    """Retorna diagnóstico completo da empresa (consulta, não executa auditoria)."""
    company = db.get(Company, company_id)
    if not company:
        raise HTTPException(status_code=404, detail="Empresa não encontrada")
...
def _get_regional_context(db: Session, category_id: int | None, city_id: int | None) -> dict | None:
    ...
    count = db.query(Audit).join(Company).filter(
    ...
    audits = db.query(Audit).join(Company).filter(
        Company.category_id == category_id,
        Company.city_id == city_id,
    ).all()
    ...
    return {
        "total_audited": count,
        "pct_without_site": sum(1 for a in audits if not a.has_whatsapp) / count,
        "pct_without_scheduling": sum(1 for a in audits if not a.has_scheduling) / count,
        "avg_score": sum(a.digital_score for a in audits) / count,
    }
