...
class CSVCollector:
    ...
    def _process_row(self, row: dict):
        ...
        if not city:
            raise ValueError(f"Cidade '{cidade_nome}' não encontrada no banco. Execute seed_geography.py primeiro.")

        category = self.db.query(Category).filter(Category.slug == categoria_slug).first()
        if not category:
            raise ValueError(f"Categoria '{categoria_slug}' não encontrada. Execute seed_categories.py primeiro.")
